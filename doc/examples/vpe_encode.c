/*
 * EMERGETECH sample for encoding with VPE suppprted avcodec API.
 * EMERGETECH sample for encoding with VPE suppprted avcodec API.
...
static const AVDictionaryEntry device_options[] = { { "priority", "vod" },
                                                    { "vpeloglevel", "0" } };
static const AVDictionaryEntry input_options[] = { { "video_size", "1280x720" },     // source file resolution
                                                   { "pixel_format", "nv12" } };     // source file pix_format
...
static int open_input_file(const char *filename)
{
    ...
    /* create decoding context: rawvideo */
...
/* This program demonstrate how vpe encoder works with avcodec APIs.
 * This example realizes encoding raw yuv video to compressed es data.
 *
 * Supported input pix_fmt format: nv12, yuv420p, p010le, yuv422p, yuv444, rgb etc.
 * Supported encoder format: h264/hevc/vp9.
 *
 * example input: 1280x720 nv12 raw data
 *                resolution/pix_fmt can be changed in input_options.
 * output: ES file
 */
int main(int argc, char **argv)
{
    ...
    if (argc <= 2) {
        av_log(NULL, AV_LOG_ERROR,
               "Usage: %s <nv12 file name> <vpe encoding codec name> <output file>\n", argv[0]);
        return 1;
    }
    ret = init_vpe_device();
    if (ret < 0) {
    ...
    /* Input file resolution and pixel format should be aligned with input_options.
     */
    ...
    /* alloc frame for rawdec output */
    frame = av_frame_alloc();
    /* alloc frame for filter graph output */
    filt_frame = av_frame_alloc();
    if (!frame || !filt_frame) {
        perror("Could not allocate frame");
        goto end;
    }
    /* alloc packet for encoder output */
        ...
        if (packet.stream_index == video_stream_index) {
            ...
            while (ret >= 0) {
                ...
                while (1) {
                    ...
                    if (ret < 0)
                        goto end;
                    /* push filtered frames to encoder and get encoded packet */
    ...
    /* flush encoder to finish encoding process */
