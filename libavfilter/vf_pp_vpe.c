/*
 * EMERGETECH VPE Post Processing Filter
 * EMERGETECH VPE Post Processing Filter
...
static void vpe_pp_picture_consume(VpePPFilter *ctx, VpiFrame *vpi_frame)
{
    VpiCtrlCmdParam cmd_param;

    // make PP release frame
    cmd_param.cmd  = VPI_CMD_PP_CONSUME;
...
static void vpe_pp_clear_unused_frames(VpePPFilter *ctx)
{
    ...
    while (cur_frame) {
        if (cur_frame->used
            ...
            vpe_pp_picture_consume(ctx, cur_frame->vpi_frame);
            cur_frame->used = 0;
        }
...
static int vpe_pp_filter_frame(AVFilterLink *inlink, AVFrame *frame)
{
    ...
    av_frame_free(&frame);
    av_frame_move_ref(out, pp_frame);
