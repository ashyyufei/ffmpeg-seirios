...
static void pool_release_buffer_vpe(void *opaque, uint8_t *data)
{
    BufferPoolEntry *buf = opaque;
    AVBufferPool *pool = buf->pool;
    ...
    ff_mutex_lock(&vpi_frame->frame_mutex);
    if (vpi_frame->vpe_frame_free && vpi_frame->free_opaque) {
        vpi_frame->vpe_frame_free(vpi_frame->free_opaque, vpi_frame);
    }
    if (vpi_frame->locked) {
        vpi_frame->locked = 0;
    }
    ff_mutex_unlock(&vpi_frame->frame_mutex);
...
static AVBufferRef *pool_alloc_buffer_vpe(AVBufferPool *pool)
{
    BufferPoolEntry *buf;
    AVBufferRef     *ret;
    ...
    ret->buffer->opaque = buf;
    ret->buffer->free   = pool_release_buffer_vpe;

    return ret;
}

AVBufferRef *av_buffer_pool_get_vpe(AVBufferPool *pool)
{
    AVBufferRef *ret;
    BufferPoolEntry *buf;
    ...
    buf = pool->pool;
    if (buf) {
        ret = av_buffer_create(buf->data, pool->size, pool_release_buffer_vpe,
                               buf, 0);
        if (ret) {
            pool->pool = buf->next;
            buf->next = NULL;
        ...
        ret = pool_alloc_buffer_vpe(pool);
    }
    ff_mutex_unlock(&pool->mutex);
